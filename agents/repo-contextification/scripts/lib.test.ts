...
describe("findHeadingOnlyGaps", () => {
  it("detects when stem content exists in body without a heading", () => {
    const content =
      "# My Project\n\nA detailed overview and description of the project.\n\n## Quick Start\n\nGet started here.";
    const missing: ExpectedSection[] = [
      { label: "overview", stems: ["overview", "descript", "about"] },
    ];
    expect(findHeadingOnlyGaps(content, missing)).toEqual(["overview"]);
  });

  it("returns empty when stem content is not found in body", () => {
    const content = "# My Project\n\n## Quick Start\n\nGet started here.";
    const missing: ExpectedSection[] = [
      { label: "overview", stems: ["overview", "descript", "about"] },
    ];
    expect(findHeadingOnlyGaps(content, missing)).toEqual([]);
  });

  it("does not match stems in heading lines", () => {
    const content = "# Overview\n\nJust some unrelated content.";
    const missing: ExpectedSection[] = [
  ...
  it("matches stem prefixes in body text", () => {
    const content =
      "# Project\n\nThis section describes the prerequisites and requirements.";
    const missing: ExpectedSection[] = [
      { label: "prerequisites", stems: ["prerequisit", "require"] },
    ];
    expect(findHeadingOnlyGaps(content, missing)).toEqual(["prerequisites"]);
  });
});
...
describe("isCacheFresh", () => {
  ...
  it("returns false when maxAgeMs is negative", () => {
    const filePath = join(tempDir, "stale.md");
    writeFileSync(filePath, "cached data");
    expect(isCacheFresh(filePath, -1)).toBe(false);
  });
...
describe("BOILERPLATE_PATTERNS", () => {
  ...
  it("matches 'generated by' text", () => {
    expect(
      BOILERPLATE_PATTERNS.some((p) =>
        p.test("Generated by cookiecutter-django"),
      ),
    ).toBe(true);
  });

  it("matches 'replace this with' text", () => {
    expect(
      BOILERPLATE_PATTERNS.some((p) =>
        p.test("Replace this with your description"),
      ),
    ).toBe(true);
  });

  it("matches 'your project name' text", () => {
    expect(
      BOILERPLATE_PATTERNS.some((p) => p.test("Enter your project name here")),
    ).toBe(true);
  });
...
describe("CI_INDICATORS", () => {
  ...
  it("includes CircleCI", () => {
    expect(CI_INDICATORS.some((i) => i.path === ".circleci/config.yml")).toBe(
      true,
    );
  });

  it("includes Travis CI", () => {
    expect(CI_INDICATORS.some((i) => i.path === ".travis.yml")).toBe(true);
  });

  it("includes Azure Pipelines", () => {
    expect(CI_INDICATORS.some((i) => i.path === "azure-pipelines.yml")).toBe(
      true,
    );
  });

  it("includes Makefile", () => {
    expect(CI_INDICATORS.some((i) => i.path === "Makefile")).toBe(true);
  });
});

describe("CI_HEADING_STEMS", () => {
  it("includes ci", () => {
    expect(CI_HEADING_STEMS).toContain("ci");
  });

  it("includes prow", () => {
    expect(CI_HEADING_STEMS).toContain("prow");
  });

  it("includes pipeline", () => {
    expect(CI_HEADING_STEMS).toContain("pipeline");
  });

  it("includes continuous", () => {
    expect(CI_HEADING_STEMS).toContain("continuous");
  });
});
...
describe("SETUP_COMMAND_PATTERNS", () => {
  ...
  it("matches go mod download", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("go mod download"))).toBe(
      true,
    );
  });

  it("matches go build", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("go build"))).toBe(true);
  });

  it("matches pip install", () => {
    expect(
      SETUP_COMMAND_PATTERNS.some((p) =>
        p.test("pip install -r requirements.txt"),
      ),
    ).toBe(true);
  });
  ...
  it("matches pip3 install", () => {
    expect(
      SETUP_COMMAND_PATTERNS.some((p) => p.test("pip3 install flask")),
    ).toBe(true);
  });

  it("matches poetry install", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("poetry install"))).toBe(
      true,
    );
  });

  it("matches cargo build", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("cargo build"))).toBe(
      true,
    );
  });

  it("matches mvn install", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("mvn install"))).toBe(
      true,
    );
  });

  it("matches gradle build", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("gradle build"))).toBe(
      true,
    );
  });

  it("matches gradlew build", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("./gradlew build"))).toBe(
      true,
    );
  });

  it("matches bundle install", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("bundle install"))).toBe(
      true,
    );
  });

  it("matches dotnet restore", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("dotnet restore"))).toBe(
      true,
    );
  });

  it("matches make install", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("make install"))).toBe(
      true,
    );
  });

  it("matches cmake", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("cmake .."))).toBe(true);
  });
  ...
  it("does not match go test", () => {
    expect(SETUP_COMMAND_PATTERNS.some((p) => p.test("go test"))).toBe(false);
  });

  it("does not match cargo test", () => {
