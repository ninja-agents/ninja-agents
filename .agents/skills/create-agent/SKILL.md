...
- **Should Codex auto-invoke this?** Or manual-only via `/command`?
  - Auto-invoke (default) — Codex detects when to use it from conversation
...
### 2.2 Agent Spec — `.Codex/agents/{name}.md`
...
### 2.3 Skill Spec — `.Codex/skills/{skill-name}/SKILL.md`
...
Include: "Tokens must be set as environment variables before launching Codex."}
...
Read `.Codex/agents/{name}.md` and follow the complete workflow described there.
...
Ignore the YAML frontmatter fields `model` and `memory` — those are Codex-specific and have no effect in Cursor.

For best results, use Codex Opus or an equivalent model in Cursor settings.
```
...
Save a **project** memory so future conversations know this agent exists. Write to the memory system at `/home/rlavi/.Codex/projects/-home-rlavi-Projects-ninja-agents/memory/`:
...
3. Next steps checklist:
   ...
   - [ ] Add MCP tool permissions to `.Codex/settings.json` if needed
   - [ ] Write tests (run with `npm run {name}:test` from the repo root)
   - [ ] Test the agent by running `/{skill-name}` (Codex) or `@{name}` (Cursor)
...
- Don't duplicate AGENTS.md content — the agent inherits it automatically
...
- Concise, keyword-rich description for Codex's skill discovery
